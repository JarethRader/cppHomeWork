...
int main(int argc, char *argv[])
{
	...
	Player playerList[MAX_NO_PLAYERS];		//Array of Player class variables
	int playerListSize = MAX_NO_PLAYERS;	//int variables for Max Size
...
void readPlayerList(ifstream &in, Player playerList[], int playerListSize)
{
	...
	for(int i = 0; i < playerListSize; i ++)
	{
		...
		getline(in, team);
		playerList[i].setTeamName(team);
		in >> points;
		getline(in, line);
		playerList[i].setPoints(points);
		in >> rebounds;
		getline(in, line);
		playerList[i].setRebounds(rebounds);
		in >> assists;
		getline(in, line);
		playerList[i].setAssists(assists);
...
void showPlayer(Player playerList[], int playerListSize, string playerName)
{
	...
	if(playerList[playerSpot].getName() == "First Last")					//Checks if playerIndex is empty or not
		return;
	else if(playerSpot == -1)												//Check if player name was found, if not, then breaks from function
	{
		cout << "Player not found" << endl;
		return;
	}
	else																	//if player was found, print out their stats
	{
		cout << "Name: " << playerList[playerSpot].getName() << endl;
		cout << "Team: " << playerList[playerSpot].getTeamName() << endl;
		cout << "Points: " << playerList[playerSpot].getPoints() << endl;
		cout << "Rebounds: " << playerList[playerSpot].getRebounds() << endl;
		cout << "Assists: " << playerList[playerSpot].getAssists() << endl;	
	}
}
...
void addPlayer(Player playerList[], int playerListSize)
{
	...
	getline(cin, last);
	name = first+last;
	playerList[spot].setName(name);
	cout << "Enter Team Name: " << endl;
	cin >> first;
	getline(cin, last);
	teamName = first+last;
	playerList[spot].setTeamName(teamName);
	cout << "Enter Points: " << endl;
	cin >> points;
	playerList[spot].setPoints(points);
	cout << "Enter Rebounds: " << endl;
	cin >> rebounds;
	playerList[spot].setRebounds(rebounds);
	cout << "Enter Assists: " << endl;
	cin >> assists;
	playerList[spot].setAssists(assists);
